 ...
 * @Description Enviar petición HTTP con ID de Quote para que se cree en Salesforce
 */
define(['N/https', 'N/record', 'N/log'], function (https, record, log) {
    ...
    function afterSubmit(context) {
        try {
            ...
            var qRecord = context.newRecord;
            var qId = qRecord.id;

            log.debug('Quote ID', qId);

            

            // Realizar petición HTTP POST al endpoint
            var response = https.get({
                url: 'https://idicotools.azurewebsites.net/api/netsuite/salesforce/quote_one/'+qId, 
                headers: {
