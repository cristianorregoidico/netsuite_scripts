    ...
    function onRequest(context) {
        if (context.request.method === 'GET') {
            ...
            if (!recordId || !messageId)  {
                log.debug("Missing recordId or messageId")
                context.response.status = 400;
                context.response.write(JSON.stringify({ success: false, message: 'recordId and messageId are required' }));
                return;
            }
            ...
            try {
                ...
                searchResults.forEach(function(result) {
                    ...
                    if (isVendorValid){
                    results.push({
                      transaction_id: result.getValue({ name: 'internalid', join: 'transaction' }),
                      ...
                      messageid: result.id, // This is correct, as result.id works for the record ID.
