...
define(['N/record', 'N/log'], function (record, log) {
    function fieldChanged(context) {
        ...
        if (context.sublistId === 'taxdetails' && context.fieldId === 'taxrate') {
            ...
            // Validar si el taxcode es un ICA
            var isIca = validateIca(parseInt(taxCode));
            log.debug('Is Ica', isIca);

            // Capturar el Tax Rate (Tasa de Impuesto)
            ...
            // Capturar el Base de Imp (Base de Impuesto)
            ...
            if (taxbasis && taxRate) {
                var taxAmount = 0;
                // Calcular impuesto según el tax code (ICA o Normal) 
                if(isIca){
...
define(['N/record', 'N/log'], function (record, log) {
    function fieldChanged(context) {
        ...
        if (context.sublistId === 'taxdetails' && context.fieldId === 'taxrate') {
            var lineIndex = context.line; // Índice de la línea actual
            log.debug('Línea Actual', lineIndex);
            // Obtener valores de Tax Basis y Tax Amount
            ...
            if (taxbasis && taxRate) {
                ...
                // Establecer el nuevo valor en la línea
                // currentRecord.setCurrentSublistValue({
                //     sublistId: 'taxdetails',
                //     fieldId: 'taxbasis',
                //     value: netAmount.toFixed(2) // Redondeamos a 2 decimales
                ...
                //     sublistId: 'taxdetails',
                //     fieldId: 'taxrate',
                //     value: taxRate.toFixed(2) // Redondeamos a 2 decimales
                // });
                currentRecord.setCurrentSublistValue({
                    sublistId: 'taxdetails',
                    fieldId: 'taxamount',
